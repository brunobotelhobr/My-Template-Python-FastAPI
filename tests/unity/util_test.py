...
def test_generator():
    ...
    assert generator.hasher(
        password=generator.password(), salt=generator.salt()
    ) != generator.hasher(password=generator.password(), salt=generator.salt())
    ...
    assert (
        len([c for c in generator.password(lower=3) if c in string.ascii_lowercase]) >= 3
    )
    assert (
        len([c for c in generator.password(lower=5) if c in string.ascii_lowercase]) >= 5
    )

    # validate is password can contains a least x upercase characters
    assert (
        len([c for c in generator.password(uper=3) if c in string.ascii_uppercase]) >= 3
    )
    assert (
        len([c for c in generator.password(uper=5) if c in string.ascii_uppercase]) >= 5
    )
    ...
    assert (
        len(
            [
                c
                for c in generator.password(numbers=3, special=3, uper=3, lower=3)
