...
class SettingsAuth(BaseModel):
    ...
    jwt_key: str = Field(title="JWT key", description="JWT key", default=running_environment.jwt_key)
    jwt_algorithm: str = Field(title="JWT algorithm", description="JWT algorithm", default="HS256")
    jwt_expiration_initial: int = Field(
        title="JWT initial expiration in minutes", description="Initial duration for a JWT", default=30
    )
    jwt_expiration_step: int = Field(
        title="JWT increment in minutes", description="JWT increment in minutes", default=30
    )
    jwt_expiration_max: int = Field(
        title="JWT max expiration in minutes", description="JWT max expiration in minutes", default=120
    )
    ...
    block_user_after_fail_attempts: int = Field(
        title="Block user after fail attempts", description="Block user after fail attempts, if 0 disabled", default=5
    )
