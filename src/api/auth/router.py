...
def login(form: OAuth2PasswordRequestFormStrict = Depends(), database: Session = Depends(get_database_session)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Bad request: Invalid credentials."
        ) from error
    # User Exists?
    user_db_query = database.query(UserORM).filter(UserORM.email == credetials.username).first()
    ...
    if hash_handler.verify_hash(password=credetials.password, hash=user_db.password_hash) is False:
        ...
        if global_settings.auth.block_user_after_fail_attempts > 0:
            if user_db_query.password_strikes >= global_settings.auth.block_user_after_fail_attempts:
...
def logout(req: Request, who: UserOut = Depends(authenticate)) -> JSONResponse:
    ...
    if req.headers.get("Authorization"):
        jwt_factory.revoke(req.headers["Authorization"])
    resp = JSONResponse(
        content=SimpleMessage(status="Sucessfully logged out.").json(), status_code=status.HTTP_302_FOUND
    )
